...
# 5. Xorg + NVIDIA ドライバー（GPU レンダリングと VSync 対応）
...
# 11. Google Chrome (Xvfbを使用した非Headlessモードによる標準的な拡張機能読み込み)
